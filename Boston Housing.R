...
qplot(x = 1:12, y = cumsum(pca$sdev)/sum(pca$sdev), geom = "line",xlab = "Number of Principal Components",ylab="Cumulative Variance explained")
#individual variance plot
qplot(x = 1:12, y = pca$sdev/sum(pca$sdev), geom = "line",xlab = "Principal Components",ylab="Individual Variance explained")
#correlation chart
chart.Correlation(df_x, histogram=TRUE, pch=25)

plot_ly(as.data.frame(pca$x), x = ~PC1, y = ~PC2, z = ~PC3, color = ~BostonHousing$medv) %>% add_markers()
...
plot_ly(aedf3, x = ~node1, y = ~node2, z = ~node3, color = ~BostonHousing$medv) %>% add_markers()


#############RECONSTRUCTION ERROR multiple k###########################
##tried both tanh and linear activation functions to show difference
...
df <- data.frame(k = c(1:9, 1:9), mse = c(xhat[1:9], ae.mse), method = c(rep("pca", 9), rep("autoencoder", 9)))
